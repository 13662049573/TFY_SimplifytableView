...
typedef NS_ENUM(NSInteger, TFY_StringPickerMode) {
    TFY_StringPickerComponentSingle,  // 单列
    TFY_StringPickerComponentMore     // 多列
};

@interface TFY_StringPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
{
...
@property (nonatomic, assign) TFY_StringPickerMode type;
...
@property (nonatomic, copy) TFY_StringResultBlock resultBlock;
@property (nonatomic, copy) TFY_StringCancelBlock cancelBlock;
@end
...
+ (void)showStringPickerWithTitle:(NSString *)title
    ...
    TFY_StringPickerView *strPickerView = [[TFY_StringPickerView alloc]initWithTitle:title dataSource:dataSource defaultSelValue:defaultSelValue isAutoSelect:isAutoSelect themeColor:themeColor resultBlock:resultBlock cancelBlock:cancelBlock];
...
- (void)configDataSource:(id)dataSource defaultSelValue:(id)defaultSelValue {
    ...
    if ([[self.dataSourceArr firstObject] isKindOfClass:[NSString class]]) {
        self.type = TFY_StringPickerComponentSingle;
    } else if ([[self.dataSourceArr firstObject] isKindOfClass:[NSArray class]]) {
        self.type = TFY_StringPickerComponentMore;
    }
    // 4. 给选择器设置默认值
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView {
    switch (self.type) {
        case TFY_StringPickerComponentSingle:
            return 1;
            break;
        case TFY_StringPickerComponentMore:
