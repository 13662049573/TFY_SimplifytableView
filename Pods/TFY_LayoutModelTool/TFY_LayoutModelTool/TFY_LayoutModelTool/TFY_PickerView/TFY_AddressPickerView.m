...
@interface TFY_AddressPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
{
...
// 显示类型
@property (nonatomic, assign) TFY_AddressPickerMode showType;
// 选中的省
...
// 选中后的回调
@property (nonatomic, copy) TFY_AddressResultBlock resultBlock;
// 取消选择的回调
@property (nonatomic, copy) TFY_AddressCancelBlock cancelBlock;
...
- (void)loadData {
    ...
    if (!self.dataSource || self.dataSource.count == 0) {
        ...
        NSString *filePath = [plistBundle pathForResource:@"TFY_City" ofType:@"plist"];
...
- (void)scrollToRow:(NSInteger)provinceIndex secondRow:(NSInteger)cityIndex thirdRow:(NSInteger)areaIndex {
    if (self.showType == TFY_AddressPickerModeProvince) {
        [self.pickerView selectRow:provinceIndex inComponent:0 animated:YES];
    } else if (self.showType == TFY_AddressPickerModeCity) {
        [self.pickerView selectRow:provinceIndex inComponent:0 animated:YES];
        [self.pickerView selectRow:cityIndex inComponent:1 animated:YES];
    } else if (self.showType == TFY_AddressPickerModeArea) {
        [self.pickerView selectRow:provinceIndex inComponent:0 animated:YES];
        [self.pickerView selectRow:cityIndex inComponent:1 animated:YES];
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView {
    switch (self.showType) {
        case TFY_AddressPickerModeProvince:
            return 1;
            break;
        case TFY_AddressPickerModeCity:
            return 2;
            break;
        case TFY_AddressPickerModeArea:
            return 3;
            break;
